...
class AutoskeletonPublicApiRadiusResolver(
    ...
    /** Returns a resolution when R1 can answer definitively (no background at all
     *  -> trivially square/`MEASURED`; a background whose outline reports a
     *  non-negative radius -> `OUTLINE`), or `null` to fall through to R3 (a
     ...
     *  `getConstantState().newDrawable().mutate()` — the exact mechanism R2's raster
     *  probe legitimately needs, since `draw(Canvas)` can have real side effects —
     *  is not achievable here at all. `getOutline(Outline)` is a different, read-only
     *  QUERY API by contract: it only populates the `Outline` instance the caller
     *  passes in and never mutates the drawable's own state, which is exactly why
     *  Android's own `View.getOutline()` default implementation calls it directly on
     *  the live `background` with no copy in between. R1 therefore reads the live
     *  drawable directly; only R2 (task 4.3) needs — and gets — an isolated copy. */
