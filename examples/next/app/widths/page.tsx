        ...
        <table className="mt-3 w-full border-collapse text-left font-mono text-xs">
          <thead>
            <tr className="border-b border-ui-line">
              <th className="py-2 pr-4 font-medium text-ui-ink-3">width bucket</th>
              <th className="py-2 pr-4 font-medium text-ui-ink-3">captured frame</th>
              <th className="py-2 font-medium text-ui-ink-3">cache key</th>
            </tr>
          </thead>
          <tbody>
            {ltrEntries.map((entry) => (
              <tr key={entry.widthBucket} className="border-b border-ui-line">
                <td className="py-2 pr-4">{entry.widthBucket}px</td>
                <td className="py-2 pr-4">
                  {entry.snapshot.frame[0]} × {entry.snapshot.frame[1]}
                </td>
                <td className="break-all py-2 text-ui-ink-3">{entry.snapshot.key}</td>
              </tr>
            ))}
          </tbody>
        </table>
